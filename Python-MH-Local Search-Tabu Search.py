...
X = pd.read_csv('Python-MH-Local Search-Tabu Search-Dataset-01.txt', sep = '\t') 
X = X.values
...
plot_tour_distance_matrix(X, lsts)

######################## Part 2 - Usage ####################################

# Load File - Coordinates (Berlin 52,  optimal = 7544.37)
Y = pd.read_csv('Python-MH-Local Search-Tabu Search-Dataset-02.txt', sep = '\t') 
Y = Y.values
...
# Call the Function
lsts = tabu_search(X, city_tour = seed, iterations = 200, tabu_tenure = 20)

# Plot Solution. Red Point = Initial city; Orange Point = Second City
plot_tour_coordinates(Y, lsts)
